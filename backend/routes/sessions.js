...
router.get("/today", protectRoute, async (req, res) => {
  try {
    ...
    const endYesterday = new Date(); endYesterday.setDate(endYesterday.getDate() - 1); endYesterday.setHours(23,59,59,999);

    const todaySessions = await StudySession.find({ user: req.user.userId, createdAt: { $gte: startToday } });
    const yesterdaySessions = await StudySession.find({ user: req.user.userId, createdAt: { $gte: startYesterday, $lte: endYesterday } });
