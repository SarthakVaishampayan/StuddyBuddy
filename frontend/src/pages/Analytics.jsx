...
const Analytics = () => {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/sessions/weekly-stats', {
          headers: { Authorization: `Bearer ${token}` }
        });
        const d = await res.json();
        if (d.success) {
          setData(d.graphData);
          // Calculate total from RAW SECONDS to avoid rounding errors
          const total = d.graphData.reduce((acc, curr) => acc + curr.rawSeconds, 0);
          setTotalSec(total);
        }
      } catch (err) {
  ...
  return (
    <div className="bg-light min-vh-100">
      ...
      <div className="p-4 px-lg-5">
        <h2 className="fw-bold mb-5 mt-3">Study Insights</h2>
        
        <div className="row g-4 mb-4">
          <div className="col-md-4">
            <div className="bg-white p-4 rounded-4 border shadow-sm">
              <div className="d-flex align-items-center gap-2 mb-2 text-primary">
                <Clock size={18}/> <span className="small fw-bold text-uppercase">Weekly Total</span>
              </div>
              <h3 className="fw-bold text-dark">{formatHms(totalSec)}</h3>
              <p className="text-muted small mb-0">Total focus time logged in last 7 days</p>
            </div>
          </div>
          <div className="col-md-4">
            <div className="bg-white p-4 rounded-4 border shadow-sm">
              <div className="d-flex align-items-center gap-2 mb-2 text-success">
                <Calendar size={18}/> <span className="small fw-bold text-uppercase">Consistency</span>
              </div>
              <h3 className="fw-bold">{data.filter(d => d.rawSeconds > 0).length} / 7 Days</h3>
              <p className="text-muted small mb-0">Days you met your study goals</p>
            </div>
          </div>
        </div>
                ...
                <XAxis 
                  ...
                  tick={({x, y, payload}) => (
                    <g transform={`translate(${x},${y})`}>
                      ...
                      <text x={0} y={28} dy={16} textAnchor="middle" fill="#6b7280" fontSize={11}>{data[payload.index]?.date}</text>
                    </g>
                  )} 
                />
                <YAxis hide={true} />
